...
A verificação concentra-se na avaliação da conformidade de um item de teste
em relação a especificações, requisitos definidos e demais artefatos de
desenvolvimento, buscando responder se o produto está sendo construído
corretamente, em conformidade com seus documentos de projeto e requisitos
estabelecidos.

A validação, por sua vez, está relacionada à aceitabilidade do item de teste
para satisfazer as necessidades dos stakeholders quando utilizado conforme
o propósito pretendido, tendo como objetivo assegurar que o produto
construído seja o produto correto do ponto de vista do usuário e do negócio.

O teste de software constitui uma das principais práticas que suportam tanto
a verificação quanto a validação, podendo ser conduzido por meio de
abordagens estáticas ou dinâmicas.

O teste estático envolve a avaliação de itens de teste sem a execução do
código, incluindo revisões, inspeções e análises estáticas automatizadas,
sendo aplicável a documentos, modelos e código-fonte ao longo do ciclo de
vida do software.

O teste dinâmico, por outro lado, requer a execução do software e a aplicação
de casos de teste sobre código executável, permitindo observar o
comportamento do sistema em tempo de execução.

As atividades de verificação e validação são fundamentais para a gestão da
qualidade, na qual o teste atua como mecanismo de controle de qualidade ao
fornecer evidências que subsidiam decisões relacionadas à aceitação e à
liberação do software \cite{jalote2005}.

A verificação e a validação estão associadas
principalmente à análise de artefatos de requisitos e testes de aceitação,
expressos por meio de especificações escritas em Behavior-Driven Development
(BDD), nas quais os cenários representam o comportamento esperado do sistema
a partir das necessidades dos stakeholders \cite{iso29119_1_2022}.
