...
Segundo \citeonline{smart2014bdd}, Dan North trabalhou com o analista de negócios Chris Matts para aplicar sua abordagem ao espaço de análise de requisitos. Juntos, eles desenvolveram o vocabulário Given-When-Then (Dado-Quando-Então). Essa notação estruturada forma a base de uma linguagem onipresente (\textit{ubiquitous language}), um conceito extraído do Domain-Driven Design (DDD), introduzido por Eric Evans em 2003. Uma linguagem ubíqua baseada no domínio do negócio permite que clientes e desenvolvedores falem o mesmo idioma sem ambiguidade \cite{Solis2011}.
...
\begin{itemize}
    ...
    \item \textbf{Identificação da Parte do Discurso (POS Tagging)}: atribui a cada palavra sua classe gramatical, como substantivo, verbo ou pronome. Essa técnica é amplamente utilizada em abordagens de análise de requisitos e testes \cite{Wang2022}.
    
    \item \textbf{Lematização}: converte palavras para sua forma canônica (\textit{lemma}), reduzindo variações morfológicas e padronizando o texto para análises posteriores. Geralmente é aplicada em conjunto com tokenização e POS tagging \cite{Rotaru2023}. 
...
Além disso, LLMs conseguem traduzir descrições textuais de cenários BDD em código de automação Cypress, incluindo step definitions e classes no padrão Page Object Model com seletores e interações de interface, demonstrando sua habilidade de transformar linguagem natural em ações concretas \cite{Nettur2025}. Esses avanços mostram que modelos de IA podem atuar como compiladores, traduzindo especificações em linguagem natural de comportamento em código executável .
