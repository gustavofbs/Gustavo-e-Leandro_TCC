...
Para responder a questão principal de pesquisa de trabalho \ref{txt:questao-pesquisa}, foi derivada uma questão específica para o estudo de caso, juntamente com as métricas que \citeonline{Nettur2025} forneceu para a sua avaliação, seguindo a abordagem GQM \ref{tab:gqm}. A metodologia GQM foi utilizada para a definição das perguntas específicas, derivadas da pergunta principal e suas respectivas métricas para conduzir o estudo, de modo a não desviar do objetivo principal e estabelecer a avaliação quantitativa ou qualitativa, de cada uma das questões de pesquisa secundárias, que agora norteiam o estudo observacional retrospectivo \cite{Basili1994}. A ISO/IEC 25010 defini testabilidade como o nível de eficácia e eficiência em que podem ser estabelecidos critérios de teste para um produto, sistema ou componente e diversos testes podem ser feitos para demonstrar que os critérios foram atendidos. No contexto desta pesquisa, a testabilidade está diretamente relacionada à eficácia das sentenças BDD, uma vez que tais características influenciam a capacidade de sistemas baseados em IA compreenderem cenários, reconhecerem elementos de interface gráfica e gerarem testes automatizados de forma eficaz.

• Questão Específica 1: Em que medida a estruturação e o detalhamento dos cenários BDD influenciam a eficácia da IA na geração de testes automatizados?

- Métrica 1.1 - Número de Métodos de Passo Gerados: mede o total de métodos de passo gerados para corresponder aos passos dos cenários BDD, considerando todos os cenários e histórias de usuário. A ideia é verificar o quanto o modelo conseguiu cobrir os passos especificados; menos métodos gerados indicam menor completude.

- Métrica 1.2 - Número de Métodos de Passo sem Implementação: mede quantos métodos de passo foram gerados, mas ficaram sem código interno. Essa métrica evidencia lacunas onde o código foi criado estruturalmente, porém não ficou funcional

- Métrica 1.3 - Número de Métodos POM Gerados: mede o total de métodos da classe Page Object Model gerados para suportar os passos dos cenários BDD. Serve para estimar o quanto o modelo produziu a camada de abstração da interface necessária à automação.

- Métrica 1.4 - Número de Métodos POM sem Implementação: mede quantos métodos POM foram gerados sem código executável. Indica incompletude na camada de interação com a aplicação.

- Métrica 1.5 - Fração de Métodos de Passo com Implementação: razão entre o número de métodos de passo implementados e o total de métodos de passo gerados, calculada por história de usuário. Formalmente:
Métodos de Passo implementados / total de Métodos de Passo gerados.

- Métrica 1.6 - Fração de Métodos de Passo sem Implementação: razão entre o número de métodos de passo sem implementação e o total de métodos de passo gerados, também por história de usuário. Formalmente:
Métodos de Passo sem implementação / total de Métodos de Passo gerados.

- Métrica 1.7 - Fração de Métodos POM com Implementação: razão entre o número de métodos POM implementados e o total de métodos POM gerados, por história de usuário. Formalmente:
métodos POM implementados / total de métodos POM gerados.

- Métrica 1.8 - Fração de Métodos POM sem Implementação: razão entre o número de métodos POM sem implementação e o total de métodos POM gerados, por história de usuário. Formalmente:
métodos POM sem implementação / total de métodos POM gerados.
