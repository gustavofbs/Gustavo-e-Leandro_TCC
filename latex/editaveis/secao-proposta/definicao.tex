...
O desenvolvimento e a manutenção do CAPJu são realizados por estudantes vinculados às disciplinas Métodos de Desenvolvimento de Software (MDS) e Engenharia de Produto de Software (EPS), ambas no contexto da organização em plataformas de código por parte da Universidade de Brasília (UnB) \footnote{\url{https://github.com/fga-eps-mds}}. Essas equipes são responsáveis pela implementação de funcionalidades, correções, testes, documentação, refinamento de requisitos e decisões arquiteturais, sempre em alinhamento com as orientações do stakeholder institucional.
...
- Métrica 1.6 - Fração de Step Methods sem Implementação: razão entre o número de métodos de passo sem implementação e o total de métodos de passo gerados, também por história de usuário. Formalmente:
