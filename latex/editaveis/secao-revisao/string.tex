...
A definição de uma estratégia de busca requer o uso de um protocolo que auxilie na delimitação precisa dos estudos relevantes. Para isso, utilizou-se o protocolo PICO, originalmente proposto na área da saúde, mas amplamente aplicável em diferentes domínios quando se busca estruturar perguntas de pesquisa. Como parte de sua adaptação ao contexto da Engenharia de Software, os elementos do PICO foram reinterpretados de forma a representar a população de estudos, a intervenção tecnológica analisada, possíveis comparações metodológicas e os resultados de interesse \cite{Pai2004}. Essa estrutura permitiu organizar a formulação da string de busca e reduzir a ocorrência de artigos irrelevantes, tornando o processo mais preciso e alinhado aos objetivos desta monografia. Uma representação do protocolo na figura \ref{fig:pico} facilita visualizar essa adaptação e compreender seu papel no delineamento da busca.
...
Para a construção da string de busca, empregou-se o operador lógico OR entre os termos de cada elemento do protocolo PICO, de modo a contemplar sinônimos e variações terminológicas amplamente utilizadas na literatura. Os elementos do PICO foram então conectados por meio do operador AND, garantindo que apenas estudos alinhados simultaneamente à população, intervenção e resultado fossem recuperados. Contudo, as primeiras execuções retornaram um volume significativo de publicações irrelevantes, o que demandou um processo iterativo de ajustes e refinamentos. Esse processo envolveu sucessivas revisões dos termos, exclusão de combinações que geravam “ruído” e inclusão de variações semânticas mais precisas, além do uso de sintaxes específicas das bases consultadas. Após diversas iterações, chegou-se à versão final da string, apresentada a seguir:
